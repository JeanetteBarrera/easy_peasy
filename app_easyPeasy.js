const http = require("http"); // requiero modulo nativo http
...
http.createServer((req, res) => {// req:request && res:response
    // defino el tipo de dato que devuelve el servidor;
    ...
    switch(req.url) {
        ...
        case "/":
            ...
            let titulos = []; // array que almacena los nombres de las peliculas
            movies.forEach((movie) => titulos.push(movie.title)); // recoore movie y se almacena en el array los titulos
            titulos.sort(); // ordena alfabeticamente el array;
			titulos.forEach((titulo) => res.write(`→ ${titulo}\n\n`));
        ...
        case "/mas-votadas":
			let votadas = movies.filter((movie) =>movie.vote_average >=7);
			res.write("Mas Votadas\n\n");
			res.write(`Total de peliculas: ${votadas.length} \n\n`);
			
			let suma = 0;
			votadas.forEach((movie) =>{
				suma = suma + movie.vote_average;
			});
			res.write(`El rating promedio: ${(suma/votadas.length).toFixed(2)}\n`);
			votadas.forEach((movie)=> {
				res.write(`→ ${movie.title} \n`);
        ...
        case "/sucursales":
			...
			theaters.forEach((sala)=> {
				res.write(`→ ${sala.name} \n`);
				res.write(`Direccion: ${sala.address} \n`);
   ...
   // res.end("mi primer servidor en Node JS con la comision 4")
}).listen(3030, "localhost", ()=>console.log("El servidor funcionando en el puerto 3030"));
